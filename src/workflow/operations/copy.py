...
# Internal
from workflow.exceptions import CollisionError, DestinationNotFoundError, InvalidItemType, InvalidSelfCopy, SameFileError, SourceNotFoundError
...
class CopyCommand(BaseCommand):
    ...
    def _validate_destination_path(self, path : str) -> Path:
        ...
        if not FileSafety.does_exists(path=destination_path):
            raise DestinationNotFoundError(f"Destination '{destination_path}' does not exist. Choose an existing folder.")
    ...
    def _mandatory_check(self):
        ...
        self._check_descendant_destination()

    def _check_descendant_destination(self):
        if FileSafety.is_relative_to(path1=self.source_path, path2=self.destination_path):
            raise InvalidSelfCopy(f"Cannot copy '{self.source_path}' into descendant '{self.destination_path}'. Choose a destination outside the source folder.")
